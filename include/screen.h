...
public:	
	...
	/* SCREEN(ATTR FColor, ATTR BColor=ATTR::BBlack);                 // 有參數建構式，傳一個參數指定前景色、背景色為黑色，傳兩個參數指定前景色、背景色 */
	SCREEN(int FColor, int BColor=0);                              // 有參數建構式，傳一個參數指定前景色、背景色為黑色，傳兩個參數指定前景色、背景色
	~SCREEN();                                                     // 解構式，釋放螢幕緩衝區記憶體，恢復黑底、白字
	int GetLins(void);                                             // 取得螢幕列數
	int GetCols(void);                                             // 取得螢幕每列字數
	void Locate(int, int);                                         // 設定游標位置
	/* void SetColor(ATTR FColor, ATTR BColor=ATTR::Reset);           // 設定顏色 */
	void SetFColor(int);
	void SetFColor(int, int, int);
	void SetBColor(int);
	void SetBColor(int, int, int);
	void SetUnderLine(bool);
	/* void SetAttr(ATTR);                                            // 設定屬性 */
...
private:
  ...
  int CurFColor, CurBColor;                                      // 儲存現有前景色、背景色	
	...
	void Init(int FColor=7, int BColor=0);                         // 螢幕物件初始化，可於初始化時設定顏色
